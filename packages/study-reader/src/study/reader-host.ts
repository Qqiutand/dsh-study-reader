...
import type { OutlineItem, SourceId } from './types.ts'
...
function readiness(source: Awaited<ReturnType<StudyService['listAllSourcesForCurrentInitiator']>>[number]) {
...
export function createStudyReaderHost(service: StudyService, principalId: string): ReaderHost {
  ...
  return {
    ...
    async getContext({ principalId: requested, signal }) {
      ...
      const sources = await service.listAllSourcesForCurrentInitiator()
    ...
    async listDocuments({ principalId: requested, query, limit, signal }) {
      signal.throwIfAborted(); if (requested !== principalId) throw new StudyError('reader principal mismatch', 'PERMISSION_DENIED'); assertPrincipal()
      const sources = await service.listSourcesForCurrentInitiator(query, limit)
    ...
    async getOutline({ principalId: requested, documentId, maxDepth, signal }) {
      signal.throwIfAborted(); if (requested !== principalId) throw new StudyError('reader principal mismatch', 'PERMISSION_DENIED'); assertPrincipal()
      const result = await service.outlineForCurrentInitiator(documentId as SourceId)
      return outlineTree(result.outline, maxDepth)
    },
    async searchPassages({ principalId: requested, query, documentIds, limit, signal }) {
      signal.throwIfAborted(); if (requested !== principalId) throw new StudyError('reader principal mismatch', 'PERMISSION_DENIED'); assertPrincipal()
      const ids = documentIds ?? (await service.listAllSourcesForCurrentInitiator()).flatMap(source => source.revisionId === undefined ? [] : [source.id])
      const passages: HostPassage[] = []
      let truncated = false
      for (const id of ids) {
        signal.throwIfAborted()
        const result = await service.searchForCurrentInitiator({ sourceId: id as SourceId, query, limit })
    ...
    async readPassage({ principalId: requested, documentId, anchor, window, signal }) {
      ...
      const info = await service.sourceInfoForCurrentInitiator(sourceId)
      let range
      if (anchor.kind === 'passage') {
        ...
        const outline = await service.outlineForCurrentInitiator(sourceId)
      ...
      const result = await service.readForCurrentInitiator({ sourceId, range }, 20_000)
