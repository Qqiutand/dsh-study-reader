...
外部 MCP 连接在 **书房 → 外部 AI 访问** 中创建。先配置一条稳定的客户端连接，再在
其中管理多个命名书单；增删或修改书单不会更换 Token，也不需要修改 Codex 配置。
选择器支持全部文献、未分类、文献文件夹、本次对话文献以及复制已有书单。MCP 提供
`reader_list_sets` 和五个只读文献工具；存在多个书单时，调用通过简短的 `setRef`
指定范围。服务端不设置调用次数预算。DSH 的 Skills、配置预设、记忆、导入和写入能力
仍留在 DSH 内部。
